...
export interface TrackingInfo {
  code: string;
  carrier: string;
  status: 'in_transit' | 'out_for_delivery' | 'delivered' | 'pending' | 'exception';
  ...
  deliveryProgress?: number; // 0-100, used for out_for_delivery animation
...
function coordsOf(city: string): [number, number] {
  return CITIES[city] || [-23.5505, -46.6333];
}

// ─── Carrier detection ─────────────────────────────────────────────────────────
export function detectCarrier(code: string): string {
...
// Deterministic pseudo-random from code string (same code → same result)
...
// ─── Mock Correios / national tracking ────────────────────────────────────────
function buildNationalTracking(code: string, carrier: string, now: Date): TrackingInfo {
  const h = hashCode(code);
  const statuses: TrackingInfo['status'][] = ['in_transit', 'out_for_delivery', 'delivered'];
  const status = statuses[h % statuses.length];

  // Pick realistic origin/destination pair
  ...
  // Transit cities between origin and destination
  const transitMap: Record<string, Record<string, string[]>> = {
    'São Paulo - SP': {
    ...
    'Fortaleza - CE': {
      'São Paulo - SP': ['Recife - PE', 'Salvador - BA', 'CTE São Paulo - SP'],
    },
    'Salvador - BA': {
      'São Paulo - SP': ['CTE São Paulo - SP'],
    },
  };

  const transits: string[] = transitMap[origin]?.[destination] ?? [];
  ...
  events.push({
    ...
    description: `Objeto postado${carrier === 'Correios' ? ' na agência dos Correios' : ` na unidade da ${carrier}`}`,
    location: origin,
    date: fmtDate(daysAgo(now, postDay + 1)),
    time: `${pad(8 + (h % 4))}:${pad((h * 7) % 60)}`,
    coords: coordsOf(origin),
  });

  events.push({
    id: String(id++),
    status: 'Em trânsito - por favor aguarde',
    description: `Objeto encaminhado de ${origin}`,
    location: origin,
    date: fmtDate(daysAgo(now, postDay)),
    time: `${pad(14 + (h % 6))}:${pad((h * 3) % 60)}`,
    coords: coordsOf(origin),
  });
  ...
  transits.forEach((city, i) => {
    events.push({
      id: String(id++),
      status: 'Em trânsito - por favor aguarde',
      description: `Objeto chegou à unidade de distribuição de ${city}`,
      location: city,
      date: fmtDate(daysAgo(now, postDay - 1 - i)),
      time: `${pad(6 + (h % 8))}:${pad((h * 11 + i * 13) % 60)}`,
      coords: coordsOf(city),
    });
  });
  ...
  if (status === 'out_for_delivery' || status === 'delivered') {
    events.push({
      ...
      date: fmtDate(daysAgo(now, 1)),
      time: `${pad(7 + (h % 3))}:${pad((h * 17) % 60)}`,
      coords: coordsOf(destination),
      isCurrentLocation: status === 'out_for_delivery',
    });
  }
  ...
  if (status === 'delivered') {
    events.push({
      ...
      location: destination,
      date: fmtDate(daysAgo(now, 0)),
      time: `${pad(10 + (h % 4))}:${pad((h * 23) % 60)}`,
      coords: coordsOf(destination),
  ...
  const routePoints: RoutePoint[] = allCities.map((city, i) => {
    let type: RoutePoint['type'];
    if (i === 0) type = 'origin';
  ...
  const currentIdx = status === 'out_for_delivery' ? allCities.length - 1 :
    status === 'in_transit' ? Math.max(1, Math.min(transits.length, allCities.length - 2)) : allCities.length - 1;
  if (routePoints[currentIdx] && status !== 'delivered') {
    routePoints[currentIdx].type = 'current';
  }

  const progress = status === 'delivered' ? 100 :
    status === 'out_for_delivery' ? 70 + (h % 25) :
    Math.round((currentIdx / (allCities.length - 1)) * 60);
  ...
  return {
    ...
    estimatedDelivery: status !== 'delivered'
      ? fmtDate(new Date(now.getTime() + 2 * 86_400_000))
      : undefined,
    origin,
    destination,
    events: [...events].reverse(),
    lastUpdate: `${fmtDate(now)} às ${fmtTime(now)}`,
    routePoints,
...
function buildInternationalTracking(code: string, carrier: string, now: Date): TrackingInfo {
  const h = hashCode(code);
  const statuses: TrackingInfo['status'][] = ['in_transit', 'out_for_delivery', 'delivered'];
  const status = statuses[h % statuses.length];
  ...
  const events: TrackingEvent[] = [
    {
      id: '1',
      status: 'Objeto postado',
      description: 'Encomenda recebida no centro de distribuição de origem',
      location: origin,
      date: fmtDate(daysAgo(now, 18)),
      time: `${pad(9 + (h % 4))}:${pad((h * 7) % 60)}`,
      coords: coordsOf(origin),
    },
    {
      id: '2',
      status: 'Aguardando despacho aduaneiro',
      description: 'Documentação submetida para processamento alfandegário de exportação',
      location: 'Hong Kong',
      date: fmtDate(daysAgo(now, 16)),
      time: `${pad(22)}:${pad((h * 3) % 60)}`,
      coords: coordsOf('Hong Kong'),
    },
    {
      id: '3',
      status: 'Em trânsito internacional',
      description: 'Objeto em voo internacional — rota Ásia › América do Sul',
      location: 'Em voo (estimado Frankfurt - Alemanha)',
      date: fmtDate(daysAgo(now, 13)),
      time: `${pad(3 + (h % 5))}:${pad((h * 11) % 60)}`,
      coords: coordsOf('Frankfurt - Alemanha'),
    },
    {
      id: '4',
      status: 'Chegou ao Brasil',
      description: 'Objeto chegou ao aeroporto de desembarque — aguardando vistoria',
      location: 'Aeroporto Internacional de Guarulhos/SP',
      date: fmtDate(daysAgo(now, 10)),
      time: `${pad(14 + (h % 6))}:${pad((h * 17) % 60)}`,
      coords: coordsOf('Aeroporto Internacional de Guarulhos/SP'),
    },
    {
      id: '5',
      status: 'Em análise aduaneira',
      description: 'Objeto sob análise pela Receita Federal do Brasil (RFB)',
      location: 'Aeroporto Internacional de Guarulhos/SP',
      date: fmtDate(daysAgo(now, 8)),
      time: `${pad(8 + (h % 3))}:${pad((h * 5) % 60)}`,
      coords: coordsOf('Aeroporto Internacional de Guarulhos/SP'),
    },
    {
      id: '6',
      status: 'Desembaraço aduaneiro realizado',
      description: 'Objeto liberado pela Receita Federal — encaminhado à unidade de distribuição',
      location: 'Receita Federal - São Paulo/SP',
      date: fmtDate(daysAgo(now, 6)),
      time: `${pad(11 + (h % 4))}:${pad((h * 13) % 60)}`,
      coords: coordsOf('Receita Federal - São Paulo/SP'),
    },
    {
      id: '7',
      status: 'Em trânsito nacional',
      description: `Objeto encaminhado dos Correios para entrega em ${destination}`,
      location: 'CDD São Paulo - SP',
      date: fmtDate(daysAgo(now, 3)),
      time: `${pad(8 + (h % 2))}:${pad((h * 19) % 60)}`,
      coords: coordsOf('CDD São Paulo - SP'),
      isCurrentLocation: status === 'in_transit',
    },
  ];
  ...
  const progress = status === 'delivered' ? 100 :
    status === 'out_for_delivery' ? 80 + (h % 15) : 45 + (h % 25);
  ...
  return {
    ...
    status,
    estimatedDelivery: status !== 'delivered'
      ? fmtDate(new Date(now.getTime() + 3 * 86_400_000))
      : undefined,
    origin,
    destination,
    events: [...events].reverse(),
    lastUpdate: `${fmtDate(now)} às ${fmtTime(hoursAgo(now, h % 3))}`,
    routePoints,
    deliveryProgress: progress,
...
export function getSavedPackages(): SavedPackage[] {
  const saved = localStorage.getItem('logitrack_packages');
  return saved ? JSON.parse(saved) : [];
}

export function savePackage(info: TrackingInfo, nickname?: string) {
  const packages = getSavedPackages();
  if (!packages.find(p => p.code === info.code)) {
    packages.push({
      code: info.code,
    ...
    localStorage.setItem('logitrack_packages', JSON.stringify(packages));
  }
}

export function removePackage(code: string) {
  localStorage.setItem(
    'logitrack_packages',
    JSON.stringify(getSavedPackages().filter(p => p.code !== code))
  );
}
...
export function getStatusText(status: TrackingInfo['status']): string {
